  ...
  return (
    <nav className="px-4 py-4 relative">
      <div className="container mx-auto">
          ...
          <div className="hidden lg:flex items-center space-x-2">
            ...
            {!session?.id ? (
              ...
              <Button
                ...
                className="px-4 py-2 rounded-full text-sm font-medium bg-red-600 text-white hover:bg-red-700 transition-all duration-200 ml-3 hover:cursor-pointer"
              >
                Logout
              </Button>
            )}
        ...
        {isMobileMenuOpen && (
          <div
            className="fixed inset-0 bg-black bg-opacity-50 z-40 md:lg:hidden"
            onClick={handleLinkClick}
          />
        )}
        ...
        {/* Mobile Menu */}
        <div
          className={`fixed top-0 right-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-50 md:lg:hidden ${
            ...
            <div className="p-4 space-y-2">
              {navItems.slice(3).map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={handleLinkClick}
                  className={`block w-full px-3 py-2 rounded-lg text-left text-sm font-medium transition-all duration-200 ${
                    isActive(item.href)
                      ? "bg-black text-white"
                      : "bg-gray-50 text-black hover:bg-gray-100"
                  }`}
